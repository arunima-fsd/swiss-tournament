...
def deletePlayers(tournamentId, playerId=None):
    ...
    if playerId:
        query = """DELETE FROM swiss_tournament.player_info
                   WHERE id = %s;"""
        conn = connect()
        cur = conn.cursor()
        ...
        cur.execute("""DELETE FROM swiss_tournament.player_info pi
                       ...
                       INNER JOIN swiss_tournament.teams t
                       ON ( pi.team_id = t.id  )
...
def deleteMatches(tournamentId, matchId=None):
    """Remove all the match records from the database.
       Deleting this automatically deletes all the records in
       'match' table as per the cascade deletion
    ARGS:
    tournamentId: used to delete the matches from a
                  particular tournament(mandatory)
    ...
    if matchId is not None:
        ...
        query = """UPDATE swiss_tournament.player_stats SET wins = 0,
                    ...
                    INNER JOIN swiss_tournament.player_info pi
                    ON ( ps.id = pi.id  )
                    INNER JOIN swiss_tournament.teams t
                    ON ( pi.team_id = t.id  )
                    WHERE tournament_id = %s;"""
...
def registerCountry(name):
    """Registers the country names belonging to various
...
# check if country already exists
    ...
    if countryExists:
        ...
        query = """INSERT INTO swiss_tournament.country( name)
...
def registerMatch(tournament_id, first_player, second_player, match_date):
    """ Registers a match in 'match_schedule' relation. This query
     just registers a match its occurence and results is recorded
     in 'match' relation  which is updated in reportMatch()
     function."""
...
def reportMatch(matchId, winner=None, loser=None):
    ...
    Here, two cases arises, first when match has got the result
    and second when its draw. In each case we are updating two
    tables namely 'player_stats' and 'match'. I assume that whenever
    a match is draw winner and loser arguments  are passed as NULL.
    We don't need a tournament id here as there is no chance that for
    two tournaments there is one player with same id.
    """
    ...
    query = """UPDATE swiss_tournament.match_schedule
               SET isfinished = True where id = %s ;"""
    data = (matchId,)
    conn = connect()
    cur = conn.cursor()
    cur.execute(query, data)
...
def displayPlayers(tournamentId):
    ...
    cur = conn.cursor()
    query = """ SELECT id, first_name, last_name, team_id,
                matches, wins, losses, draws, total_points
                FROM swiss_tournament.player_standings
                WHERE tournament_id = %s """
    data = (tournamentId,)
    cur.execute(query, data)
...
def displayMatches(tournamentId):
    ...
    conn = connect()
    cur = conn.cursor()
    query = """SELECT ms.id, ms.first_player, pi.first_name , pi.last_name,
               ms.second_player, pi1.first_name, pi1.last_name,
               ms.match_date, ms.isfinished
               FROM swiss_tournament.match_schedule ms
	           INNER JOIN swiss_tournament.player_info pi
               ON ( ms.first_player = pi.id  )
	           INNER JOIN swiss_tournament.player_info pi1
               ON ( ms.second_player = pi1.id  )
               WHERE tournament_id = %s"""
    data = (tournamentId,)
    cur.execute(query, data)
    matches_list = []
...
def getAllRounds(tournamentId):
    """This function fetches all the round numbers for
       which the matches are already scheduled."""
